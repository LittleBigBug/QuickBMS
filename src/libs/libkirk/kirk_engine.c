...
int kirk_CMD0(u8* outbuff, u8* inbuff, int size, int generate_trash)
{
	...
	//ENCRYPT DATA
	AES_ctx k1;
	libkirk_AES_set_key(&k1, keys->AES, 128);
	
	libkirk_AES_cbc_encrypt(&k1, inbuff+sizeof(KIRK_CMD1_HEADER)+header->data_offset, outbuff+sizeof(KIRK_CMD1_HEADER)+header->data_offset, chk_size);
	
	//CMAC HASHES
	AES_ctx cmac_key;
	libkirk_AES_set_key(&cmac_key, keys->CMAC, 128);
	    
	u8 cmac_header_hash[16];
	u8 cmac_data_hash[16];
		
	libkirk_AES_CMAC(&cmac_key, outbuff+0x60, 0x30, cmac_header_hash);
	
	libkirk_AES_CMAC(&cmac_key, outbuff+0x60, 0x30 + chk_size + header->data_offset, cmac_data_hash);
	...
	libkirk_AES_cbc_encrypt(&aes_kirk1, inbuff, outbuff, 16*2);
...
int kirk_CMD1(u8* outbuff, u8* inbuff, int size, int do_check)
{
	...
	libkirk_AES_cbc_decrypt(&aes_kirk1, inbuff, (u8*)&keys, 16*2); //decrypt AES & CMAC key to temp buffer
	...
	libkirk_AES_cbc_decrypt(&k1, inbuff+sizeof(KIRK_CMD1_HEADER)+header->data_offset, outbuff, header->data_size);	
...
int kirk_CMD4(u8* outbuff, u8* inbuff, int size)
{
	...
	//Set the key
	AES_ctx aesKey;
	libkirk_AES_set_key(&aesKey, key, 128);
	libkirk_AES_cbc_encrypt(&aesKey, inbuff+sizeof(KIRK_AES128CBC_HEADER), outbuff+sizeof(KIRK_AES128CBC_HEADER), size);
...
int kirk_CMD7(u8* outbuff, u8* inbuff, int size)
{
	...
	libkirk_AES_set_key(&aesKey, key, 128);
 	...
 	libkirk_AES_cbc_decrypt(&aesKey, inbuff+sizeof(KIRK_AES128CBC_HEADER), outbuff, size);
...
int kirk_CMD10(u8* inbuff, int insize)
{
	...
	if(header->mode == KIRK_MODE_CMD1)
	{
        ...
        libkirk_AES_cbc_decrypt(&aes_kirk1, inbuff, (u8*)&keys, 32); //decrypt AES & CMAC key to temp buffer
		...
		libkirk_AES_CMAC(&cmac_key, inbuff+0x60, 0x30, cmac_header_hash);
	
		//Make sure data is 16 aligned
		int chk_size = header->data_size;
		if(chk_size % 16) chk_size += 16 - (chk_size % 16);
		libkirk_AES_CMAC(&cmac_key, inbuff+0x60, 0x30 + chk_size + header->data_offset, cmac_data_hash);
...
int kirk_CMD11(u8* outbuff, u8* inbuff, int size)
{
    ...
    SHA1Context sha;
    libkirk_SHA1Reset(&sha);
    size <<= 4;
    size >>= 4;
	size = size < header->data_size ? size : header->data_size;
    libkirk_SHA1Input(&sha, inbuff+sizeof(KIRK_SHA1_HEADER), size);
...
int kirk_init()
{
    libkirk_AES_set_key(&aes_kirk1, kirk1_key, 128);
