/*-------------------------------------------------*/
/* GRZipII/libGRZip compressor             CRC32.c */
/* CRC32 Computing Functions                       */
/*-------------------------------------------------*/
...
#define CRC32_MaxByte 256

uint32 CRC32Table[CRC32_MaxByte] = {
...
uint32 GRZip_GetCRC32(uint8 * Input,sint32 Size)
{
  sint32 i;
  ...
  {
    CRC32^=*(uint32 *)Input;
    CRC32=CRC32Table[(uint8)CRC32]^(CRC32>>8);
    CRC32=CRC32Table[(uint8)CRC32]^(CRC32>>8);
    CRC32=CRC32Table[(uint8)CRC32]^(CRC32>>8);
    CRC32=CRC32Table[(uint8)CRC32]^(CRC32>>8);
    Input+=4;Size-=4;
  ...
  for (i=0;i<Size;i++)
    CRC32=CRC32Table[(uint8)(CRC32^Input[i])]^(CRC32>>8);
  return (~CRC32);
}
