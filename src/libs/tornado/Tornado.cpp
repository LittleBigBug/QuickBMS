...
static const int MAX_MATCHES = 2048+256+16;  // maximum length of hash row plus a few low-order matches from auxiliary MFs
...
static const int default_Tornado_method = 5;
...
static const int table_dist=256*1024, table_shift=128;
...
int mycallback (const char *what, void *buf, int size, void *r_) {
    ...
    tornado_auxdata_t       *tornado_auxdata = (tornado_auxdata_t *)r_;
    ...
    if (strequ(what,"myinit_input")) {
        ...
        if(tornado_auxdata && tornado_auxdata->data && (tornado_auxdata->i < tornado_auxdata->size)) {
            // it just reads 2 and then 4 bytes
            memcpy(buf, tornado_auxdata->data + tornado_auxdata->i, size);
            tornado_auxdata->i += size;
        } else {
...
extern "C" int tornado_decompress(unsigned char *in, int insz, unsigned char *out, int outsz, int algo) {
    tornado_auxdata_t   tornado_auxdata;
    unsigned char   header[2 + 4];

    memset(&tornado_auxdata, 0, sizeof(tornado_auxdata));
    if(algo > STORING) {    // BYTECODER is 1
        header[0] = algo;
        header[1] = 2;
        *(int *)(header + 2) = (outsz < 0x8000) ? 0x8000 : outsz;
        tornado_auxdata.data = header;
        tornado_auxdata.size = sizeof(header);
    }

    mycallback("myinit_input", in, insz, NULL);
    mycallback("myinit_output", out, outsz, NULL);
    if(tor_decompress(&mycallback, &tornado_auxdata, NULL, 0) != FREEARC_OK) return -1;
