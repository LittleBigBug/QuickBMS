...
*(ctx->skey1+3)&=0x7fffffff;
*(ctx->skey2+3)&=0x3fffffff;
...
u = _mm_load_si128((__m128i*)(ctx->skey1));
u = _mm_slli_si128(u,1);
_mm_store_si128((__m128i*)(ctx->skey1),u);
...
u = _mm_load_si128((__m128i*)(ctx->skey2));
u = _mm_slli_si128(u,1);
_mm_store_si128((__m128i*)(ctx->skey2),u);
...
*(ctx->skey1+3)&=0x3fffffff;

int n;
n=1+(d&15);

temp = *(ctx->ckey1+3)>>(32-n);
temp = temp^(temp<<3);

v=_mm_load_si128((__m128i*)(ctx->ckey1));

u=_mm_slli_epi32(v,n);

v=_mm_srli_epi32 (v,32-n);

v=_mm_slli_si128(v,4);

u=_mm_or_si128 (u, v);

_mm_store_si128((__m128i*)(ctx->ckey1),u);
...
temp = *(ctx->ckey2+3)>>(32-n);
temp=temp^(temp<<5)^(temp<<7);

v=_mm_load_si128((__m128i*)(ctx->ckey2));
...
_mm_store_si128((__m128i*)(ctx->ckey2),u);
...
u=_mm_load_si128((__m128i*)(ctx->var1));
v=_mm_load_si128((__m128i*)(ctx->ckey1));

u=_mm_xor_si128(u,v);

_mm_store_si128 ((__m128i*)(ctx->var1),u);

u=_mm_load_si128((__m128i*)(ctx->var2));
v=_mm_load_si128((__m128i*)(ctx->ckey2));

u=_mm_xor_si128(u,v);

_mm_store_si128 ((__m128i*)(ctx->var2),u);
