 ...
 */
#define ECRYPT_NAME "ZK-Crypt-v1"                 /* [edit] */ 
...
/*
We allow here 128-bit key length only
*/
#define ECRYPT_MAXKEYSIZE 128                  /* [edit] */
#define ECRYPT_KEYSIZE(i) (128 + (i)*32)       /* [edit] */
/*
We allow here 128-bit maximal IV length.
Starting from 64 bits minnimum and going up to 128 in steps of 32 bits.
*/
#define ECRYPT_MAXIVSIZE 128                   /* [edit] */
#define ECRYPT_IVSIZE(i) (64 + (i)*32)         /* [edit] */
...
typedef struct
{
    ...
    u8  iv          [ECRYPT_MAXIVSIZE>>3] ; /* >>3 means /8 in bytes */
    u8  key         [ECRYPT_MAXKEYSIZE>>3];
    u8  session_key [ECRYPT_MAXKEYSIZE>>3];

    u32	top_control;      /* tier control */												
    u32	mid_control;																		
    u32	bot_control;																		
    u32	com_control;      /* common control */												
																							
    u32	p_random_clock;   /* 4C */															
																							
    u32	control_cfg_FF;   /* control config FF 13C */										
																							
    u32	top_cipher_word;  /* initial value for nLSFRs from 128 bits crypto key preload */	
    u32	mid_cipher_word;																	
    u32	bot_cipher_word;																	
																							
    u32	top_result;       /* tier result */													
    u32	mid_result;																			
    u32	bot_result;																			
																							
    u32	bank_result;      /* majority 2/3 result */ /* old version: XORed three results */	
																							
    u32	hash_control;     /* ODDNs */														
    u32	hash_result;																		
																							
    u32	imm_data;         /* immunizer data FFs */											
    u32	imm_result;       /* immunizer result */											
																							
    u32	data_result;																		
    u32	message_in;	
    u32 prev_data_result;
																							
    u32	feedback_word;    /* feedback word store */											
																							
    u32	shift_reg  [6]; /* = { 0,  0,  0,  0,  0,  0};  shift registers bank */				
																							
    u32	mjuggle_hash_toggle; /* 4C */														
    u32	hash_vector;																		
    ...
    u32	top_cnt;      /* control counter 4bit */											
    u32	mid_cnt;																			
    u32	bot_cnt;																			
																							
    u32	top_sfr3;     /* control LSFR 3bit */												
    u32	mid_sfr5;     /* control LSFR 5bit */												
    u32	bot_sfr6;     /* control LSFR 6bit */												
    ...
    u32 type_of_operation; /* 0 - RNG, 1(default) - Stream Cypher, 2 - MAC */
...
#else /*if ECRYPT_HAS_SINGLE_BYTE_FUNCTION is not defined*/
...
#endif /*ECRYPT_HAS_SINGLE_BYTE_FUNCTION*/
...
ZK-Comment:
    ...
    The ZK-Crypt cipher has additional options:
    1. Multisample - encreasing number of sampleas between outputs.
    2. Encreased key size.
    We are not implementing those options here and compeeting 
    using the simplest variant.
