...
/*	00B	&	00S	&	00F	&	1S	*/
{
	...
	/* In HW the clock cycle (Left Hand Side) happens in parrallel to the nLFSR Bank
		...
		So changes in the feedback due to the 'Right Hand side' should be propogated
...
/* 11P	11S2	34	*/
{
	...
	{	/* MAC feedback takes the input xors it with existing value and return it	*/
		ctx->sttFeedBack = in ^ lclMACstorage;
		lclMACstorage = in;
...
/*	10P	&	10S1	&	10S3	&	*/
{
	...
	else
	{
		...
		/*bit=NOT(XOR(TOP BRN, MID BRN, BOT BRN))	*/
		...
		/* bit7=TOP BRN OR bit1	*/
		fBit7 = (getBitAtLocation(topClockRes, OUTP_CLOCKS_BRN) && BIT_ONE) | fBit1;

		/* bit8=MID BRN OR bit2	*/
		fBit8 = (getBitAtLocation(midClockRes, OUTP_CLOCKS_BRN) && BIT_ONE) | fBit2;

		/* bit9=BOT BRN OR bit3	*/
		fBit9 = (getBitAtLocation(botClockRes, OUTP_CLOCKS_BRN) && BIT_ONE) | fBit3;
