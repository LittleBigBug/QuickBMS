...
/* Header file for "Polar Bear" stream cipher           */
 ...
 */
#define ECRYPT_NAME "Polar-Bear-v2"    /* [edit] */ 
...
#define ECRYPT_MAXIVSIZE 248                   /* [edit] */
#define ECRYPT_IVSIZE(i) ((i)*8)               /* [edit] */
...
typedef struct
{
  aes_ctx ks;            /* The fixed key schedule */
	                     /* State info :           */
  int ivsize;			 /* size of IV could vary from packet to packet, but here it is fixed */
  u8 D8init[256];		 /* The permutated table          */
  u8 D8[256];		     /* Current table          */
  u32 R0[10], R1[10];      /* LFSRs                  */
} ECRYPT_ctx;
