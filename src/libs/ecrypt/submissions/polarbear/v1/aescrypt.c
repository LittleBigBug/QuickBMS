...
/* 09/08/05: Code modified by M Näslund for ECRYPT use in connection to "Polar Bear" cipher */
...
{   aes_32t        locals(b0, b1);
    ...
    round(fwd_rnd,  b1, b0, kp +     nc);
    round(fwd_rnd,  b0, b1, kp + 2 * nc); 
    round(fwd_rnd,  b1, b0, kp + 3 * nc);
    round(fwd_rnd,  b0, b1, kp + 4 * nc); 
	round(fwd_rnd,  b1, b0, kp + 5 * nc); 
