...
int ARIA_EncKeySetup(const Byte *w0, Byte *e, int keyBits) {
...
int ARIA_DecKeySetup(const Byte *w0, Byte *d, int keyBits) {
	int  i, j, R;
	Byte t[16];
  
	R=ARIA_EncKeySetup(w0, d, keyBits);
	for (j = 0; j < 16; j++){
		t[j] = d[j];
		d[j] = d[16*R + j];
		d[16*R + j] = t[j];
...
void ARIA_Crypt(const Byte *p, int R, const Byte *e, Byte *c)
{
	int i, j;
	Byte t[16];
