...
static void enc(const u4byte in_blk[4], u4byte out_blk[4]);

u4byte *frog_set_key(const u4byte in_key[], const u4byte key_len)
{   u4byte  i, j, k;
    u1byte  kb[32];
...
void frog_encrypt(const u4byte in_blk[4], u4byte out_blk[4])
{   u4byte  i, k;
    u1byte  blk[16], *xp, *sp, *pp;
...
void frog_decrypt(const u4byte in_blk[4], u4byte out_blk[4])
{   s4byte  i;
