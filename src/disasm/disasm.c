...
static int Tcopycase(tchar *dest,int n,const tchar *src,int lowercase) {
  ...
  for (i=0; i<n-1; i++) {
    if (*src==T('\0')) break;
    if (lowercase)
      *dest++=cvtlower[(unsigned char)(*src++)];        // Much faster than call to tolower()
    else
      *dest++=*src++;
    ;
  };
...
ulong Disasm(uchar /*const*/ *cmd,ulong cmdsize,ulong ip,t_disasm *da,
  ...
  int i,j,k,q,noperand=0,nout,enclose,vexreg=0,success,cfill=0,ofill;
  ulong m,n,u,prefix,prefixmask,code,arg,cmdtype,datasize=0;
  ulong vex,vexlead;
  t_imdata im;
