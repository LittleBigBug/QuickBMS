...
int ideal = 0;                // Force IDEAL decoding mode
//int lowercase = 0;            // Force lowercase display
int tabarguments = 0;         // Tab between mnemonic and arguments
//int extraspace = 0;           // Extra space between arguments
int putdefseg = 0;            // Display default segments in listing
//int showmemsize = 1;          // Always show memory size
int shownear = 0;             // Show NEAR modifiers
int shortstringcmds = 0;      // Use short form of string commands
int sizesens = 0;             // How to decode size-sensitive mnemonics
int symbolic = 0;             // Show symbolic addresses in disasm
int farcalls = 1;             // Accept far calls, returns & addresses
int decodevxd = 1;            // Decode VxD calls (Win95/98)
int privileged = 1;           // Accept privileged commands
int iocommand = 1;            // Accept I/O commands
int badshift = 1;             // Accept shift out of range 1..31
int extraprefix = 1;          // Accept superfluous prefixes
int lockedbus = 1;            // Accept LOCK prefixes
int stackalign = 1;           // Accept unaligned stack operations
int iswindowsnt = 1;          // When checking for dangers, assume NT
...
static uchar     *asmcmd;              // Pointer to 0-terminated source line
static int       scan;                 // Type of last scanned element
static int       prio;                 // Priority of operation (0: highest)
static uchar     sdata[TEXTLEN];       // Last scanned name (depends on type)
static long      idata;                // Last scanned value
static long      double fdata;         // Floating-point number
static uchar     *asmerror;            // Explanation of last error, or NULL

uchar *mystrupr(uchar *str) {
    uchar   *p;
...
static void Scanasm(int mode) {
  ...
  if (isalpha(*asmcmd) || *asmcmd=='_' || *asmcmd=='@') {
    ...
    for (j=0; j<=8; j++) {
      ...
      return; };
    for (j=0; j<6; j++) {
      if (strcmp(s,_segname[j])!=0) continue;
    ...
    for (j=0; j<=8; j++) {
      ...
      return; };
    for (j=0; j<=8; j++) {
      if (strcmp(s,_mmxname[j])!=0) continue;
      idata=j; scan=SCAN_MMX;          // MMX register
      return; };
    for (j=0; j<=8; j++) {
      if (strcmp(s,_crname[j])!=0) continue;
      idata=j; scan=SCAN_CR;           // Control register
      return; };
    for (j=0; j<=8; j++) {
      if (strcmp(s,_drname[j])!=0) continue;
      idata=j; scan=SCAN_DR;           // Debug register
      return; };
    for (j=0; j<sizeof(_sizename)/sizeof(_sizename[0]); j++) {
      if (strcmp(s,_sizename[j])!=0) continue;
