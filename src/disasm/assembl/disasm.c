...
void *assemble_strlwr(void *str) {
    uchar   *p;
    for(p = (uchar *)str; *p; p++) *p = tolower(*p);
    return(str);
}
...
static void DecodeRG(int idx,int xdatasize,int type) {
  ...
  if (mode>=DISASM_FILE) {
    ...
    if (lowercase) assemble_strlwr(name);
...
static void Memadr(int defseg,const uchar *descr,int offset,int dsize) {
  ...
  if (showmemsize!=0 || (da->cmdtype & C_TYPEMASK)==C_MMX ||
    ...
    if (dsize<sizeof(_sizename)/sizeof(_sizename[0]))
      n+=sprintf(pr+n,"%s %s",_sizename[dsize],(ideal==0?"PTR ":""));
    else
      n+=sprintf(pr+n,"(%i-BYTE) %s",dsize,(ideal==0?"PTR ":""));
    ;
  };
  if ((putdefseg!=0 || seg!=defseg) && seg!=SEG_UNDEF)
    n+=sprintf(pr+n,"%s:",_segname[seg]);
  if (ideal==0) pr[n++]='[';
  n+=sprintf(pr+n,"%s",descr);
  if (lowercase) assemble_strlwr(pr);
  if (offset==0L) {
    if (*descr=='\0') pr[n++]='0'; }
  else {
...
static void DecodeSG(int idx) {
  ...
  if (mode>=DISASM_FILE) {
    i=sprintf(da->result+nresult,"%s",_segname[idx]);
    if (lowercase) assemble_strlwr(da->result+nresult);
    nresult+=i;
  };
};
...
static void DecodeCR(int idx) {
  ...
  if (mode>=DISASM_FILE) {
    idx=(idx>>3) & 0x07;
    nresult+=sprintf(da->result+nresult,"%s",_crname[idx]);
    if (lowercase) assemble_strlwr(da->result+nresult);
  };
};
...
static void DecodeDR(int idx) {
  ...
  if (mode>=DISASM_FILE) {
    idx=(idx>>3) & 0x07;
    i=sprintf(da->result+nresult,"%s",_drname[idx]);
    if (lowercase) assemble_strlwr(da->result+nresult);
    nresult+=i;
  };
};
...
ulong olly_Disasm(uchar *src,ulong srcsize,ulong srcip,
  ...
  if (repeated) {
    if (mode>=DISASM_FILE) {
      ...
      da->nprefix=1;
      switch (cmd[-(long)u]) {
        case 0x26: pname=(uchar *)(_segname[SEG_ES]); break;
        case 0x2E: pname=(uchar *)(_segname[SEG_CS]); break;
        case 0x36: pname=(uchar *)(_segname[SEG_SS]); break;
        case 0x3E: pname=(uchar *)(_segname[SEG_DS]); break;
        case 0x64: pname=(uchar *)(_segname[SEG_FS]); break;
        case 0x65: pname=(uchar *)(_segname[SEG_GS]); break;
      ...
      if (lowercase) assemble_strlwr(da->result);
