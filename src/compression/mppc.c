...
#define TAG 0
#define WLog_ERR(X,Y)   fprintf(stderr, "%s\n", Y)
...
static MPPC_CONTEXT* mppc_context_new(DWORD CompressionLevel, BOOL Compressor)
{
	...
	if (mppc)
	{
        ...
        mppc_set_compression_level(mppc, CompressionLevel);
...
int mppc_unpack(unsigned char *in, int insz, unsigned char *out, int outsz, int level) {
    ...
    ctx = mppc_context_new(level, 0);
    if(!ctx) return -1;
    ret = mppc_decompress(ctx, in, insz, &DstData, &DstSize, PACKET_COMPRESSED | PACKET_AT_FRONT | PACKET_FLUSHED);
    if(ret >= 0) {
        if(DstSize > outsz) {
            ret = -1;
        } else {
            outsz = DstSize;
            memcpy(out, DstData, outsz);
            ret = outsz;
        }
    }
    mppc_context_free(ctx);
    return ret;
}
...
int mppc_pack(unsigned char *in, int insz, unsigned char *out, int outsz, int level) {
    MPPC_CONTEXT    *ctx;
    int     ret;
    BYTE    *DstData    = out;  // uses external buffer
    UINT32  DstSize     = outsz;
    UINT32  Flags       = 0;

    ctx = mppc_context_new(level, 1);
    if(!ctx) return -1;
    ret = mppc_compress(ctx, in, insz, &DstData, &DstSize, &Flags);
    if(ret >= 0) {
        if(DstSize > outsz) {
            ret = -1;
        } else {
