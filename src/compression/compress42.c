...
// Note: MAXSEG_64K is not enabled, good.
...
#	if BITS == 16
//just in case #		define	MAXSEG_64K
#	endif
#	undef	BYTEORDER
...
//static char 			ofname[MAXPATHLEN];	/* Output filename								*/
...
static int   	compress		(int,int);
static int   	decompress		(int,int);
#define read_error()        return -1   //static void  	read_error		ARGS((void));
#define write_error()       return -1   //static void  	write_error		ARGS((void));
#define abort_compress()    return -1   //static void 	abort_compress	ARGS((void));
...
static ncompress_io_t   ncompress_input;
static ncompress_io_t   ncompress_output;
static int xread(int X, void *buff, int len) {
    int     t = ncompress_input.size - ncompress_input.i;
    if(len > t) len = t;
    memcpy(buff, ncompress_input.data + ncompress_input.i, len);
    ncompress_input.i += len;
    return len;
}
static int xwrite(int X, void *buff, int len) {
    int     t = ncompress_output.size - ncompress_output.i;
    if(len > t) len = t;
    memcpy(ncompress_output.data + ncompress_output.i, buff, len);
    ncompress_output.i += len;
    return len;
}
 ...
 */ 
int
ncompress_main(unsigned char *in, int insz, unsigned char *out, int outsz, int do_decomp) {

    ncompress_input.data    = in;
    ncompress_input.i       = 0;
    ncompress_input.size    = insz;
    ncompress_output.data   = out;
    ncompress_output.i      = 0;
    ncompress_output.size   = outsz;
        ...
        if(t < 0) return -1;
		return ncompress_output.i; //bytes_out;
	}
...
compress(fdin, fdout)
	...
	int		fdout;
	{
		...
		while ((rsize = xread(fdin, inbuf, IBUFSIZ)) > 0)
		{
			if (bytes_in == 0)
			{
				fcode.e.ent = inbuf[0];
			...
			do
			{
				...
				{
					if (xwrite(fdout, outbuf, OBUFSIZ) != OBUFSIZ)
						write_error();
...
endlop:			if (fcode.e.ent >= FIRST && rpos < rsize)
		...
		if (xwrite(fdout, outbuf, (outbits+7)>>3) != (outbits+7)>>3)
			write_error();
...
decompress(fdin, fdout)
	...
	int		fdout;
	{
		...
		while (insize < 3 && (rsize = xread(fdin, inbuf+insize, IBUFSIZ)) > 0)
			insize += rsize;
...
resetbuf:	;
			...
			{
				if ((rsize = xread(fdin, inbuf+insize, IBUFSIZ)) < 0)
					read_error();
...

				{
					...
					if (outpos+(i = (de_stack-stackp)) >= OBUFSIZ)
					{
						do
						{
							...
							if (outpos >= OBUFSIZ)
							{
								if (xwrite(fdout, outbuf, outpos) != outpos)
									write_error();
		...
		if (outpos > 0 && xwrite(fdout, outbuf, outpos) != outpos)
			write_error();
