...
static int get_bit (unsigned char *fin) {
	int t;
...
int unq3huff( unsigned char *in, int insz, unsigned char *out, int outsz ) {
	...
	for ( j = 0; (bloc >> 3) < insz; j++ ) {
		ch = 0;
		Huff_Receive(huff.tree, &ch, in);				/* Get a character */
        ...
        if(j >= outsz) return(-1);
		out[j] = ch;									/* Write symbol */

		Huff_addRef(&huff, (unsigned char)ch);								/* Increment node */
	}
    return(j);
}
...
int q3huff( unsigned char *buffer, int size, unsigned char *seq ) {
	int			i, ch;
	huff_t		huff;
