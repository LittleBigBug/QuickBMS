...
// increased length_* arrays, added sourceLen check, useless zeroes, support for swapped btype and so on
#define ALUIGI_30   288 // the original is 30, this is probably the max
...
static void tinf_build_bits_base(unsigned char *bits, unsigned short *base, int delta, int first)
{
   ...
   for (i = 0; i < ALUIGI_30 - delta; ++i) bits[i + delta] = i / delta;

   /* build base table */
   for (sum = first, i = 0; i < ALUIGI_30; ++i)
   {
      base[i] = sum;
      sum += 1 << bits[i];
   }
...
static int tinf_getbit(TINF_DATA *d)
{
   ...
   if (!d->bitcount--)
   {
      ...
      if(d->source >= d->source_limit) {
        d->tag = 0;
        d->bitcount = 0;
        return 0;    // better than an access violation
      }
      d->tag = *d->source++;
      d->bitcount = 7;
   }
...
int tinf_uncompress(void *dest, unsigned int *destLen,
                    ...
                    const char *swapped_btype)
{
   if(!swapped_btype) swapped_btype = "012";
   TINF_DATA d;
   int bfinal;
   ...
   d.bitcount = 0;
   d.source_limit = d.source + sourceLen;
   ...
   *destLen = 0;
   ...
   do {
      ...
      /* decompress block */
      if(btype == (swapped_btype[0]-'0')) {
         /* decompress uncompressed block */
         res = tinf_inflate_uncompressed_block(&d);
      } else if(btype == (swapped_btype[1]-'0')) {
         /* decompress block with fixed huffman trees */
         res = tinf_inflate_fixed_block(&d);
      } else if(btype == (swapped_btype[2]-'0')) {
      ...
      if((d.source >= d.source_limit) && !d.bitcount) bfinal = 1; // forced set
