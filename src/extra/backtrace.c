...
#define BACKTRACE_BUFFER_MAX (16*1024)
...
_backtrace(struct output_buffer *ob, struct bfd_set *set, int depth , LPCONTEXT context)
{
	...
	while(StackWalk(IMAGE_FILE_MACHINE_I386, 
		...
		void* module_base = (void *)SymGetModuleBase(process, frame.AddrPC.Offset);
...
static LPTOP_LEVEL_EXCEPTION_FILTER g_backtrace_prev = NULL;
...
{
	if (g_backtrace_output == NULL) {
		g_backtrace_output = malloc(BACKTRACE_BUFFER_MAX);
		g_backtrace_prev = SetUnhandledExceptionFilter(exception_filter);
	}
}
...
{
	if (g_backtrace_output) {
		free(g_backtrace_output);
		SetUnhandledExceptionFilter(g_backtrace_prev);
		g_backtrace_prev = NULL;
